...
def _init_worker(info: Dict):
    ...
    for key, item in info.items():
        ...
        global_shared_arrays[key] = np.ndarray(item['shape'], dtype=item['dtype'], buffer=shm.buf)
...
def _process_chunk(chunk_info: Tuple[int, int, str]):
    ...
    if index_type == "NDVI":
        ...
        result_tile = np.divide(b8_tile - b4_tile, denominator, where=denominator > 1e-6, out=np.zeros_like(b4_tile))
        ...
        result_tile = np.divide(b8_tile - b11_tile, denominator, where=denominator > 1e-6, out=np.zeros_like(b8_tile))
        output_arr[start_row:end_row] = result_tile
...
def calculate_index(
    ...
    print(f"Starting calculation for: {index_type} using CPU ({n_jobs} processes, Shared Memory)...")
    ...
    try:
        for name, arr in input_arrays.items():
            ...
            shared_arr[:] = arr[:]
            shm_list.append(shm)
            worker_info[name] = {'name': shm.name, 'shape': arr.shape, 'dtype': arr.dtype}

        output_arr_template = np.zeros((h, w), dtype=np.float32)
        shm_out = SharedMemory(create=True, size=output_arr_template.nbytes)
        worker_info[output_name] = {'name': shm_out.name, 'shape': output_arr_template.shape, 'dtype': output_arr_template.dtype}
        shm_list.append(shm_out)

        row_indices = np.array_split(np.arange(h), n_jobs)
        tasks = [(chunk[0], chunk[-1] + 1, index_type) for chunk in row_indices if len(chunk) > 0]

        with ProcessPoolExecutor(max_workers=n_jobs, initializer=_init_worker, initargs=(worker_info,)) as executor:
            list(executor.map(_process_chunk, tasks))

        final_result = np.copy(np.ndarray(output_arr_template.shape, dtype=output_arr_template.dtype, buffer=shm_out.buf))
